...
static int read_image_stb(const char* path, image_buffer_t* image)
{
    ...
    unsigned char* pixeldata = stbi_load(path, &w, &h, &source_channels, output_channels);
    if (!pixeldata) {
        printf("error: read image %s fail\n", path);
        return -1;
    }
    (void)source_channels;
    int size = w * h * output_channels;
...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    int src_wstride = srcWidth;
    int src_hstride = srcHeight;
    int dst_wstride = dstWidth;
    int dst_hstride = dstHeight;

    if (src_img->width_stride > 0) {
        src_wstride = src_img->width_stride;
    }
    if (src_img->height_stride > 0) {
        src_hstride = src_img->height_stride;
    }
    if (dst_img->width_stride > 0) {
        dst_wstride = dst_img->width_stride;
    }
    if (dst_img->height_stride > 0) {
        dst_hstride = dst_img->height_stride;
    }
    ...
    if (use_handle) {
        ...
        rga_buf_src = wrapbuffer_handle(rga_handle_src, srcWidth, srcHeight, srcFmt, src_wstride, src_hstride);
    } else {
        if (src_phy != NULL) {
            rga_buf_src = wrapbuffer_physicaladdr(src_phy, srcWidth, srcHeight, srcFmt, src_wstride, src_hstride);
        } else if (src_fd > 0) {
            rga_buf_src = wrapbuffer_fd(src_fd, srcWidth, srcHeight, srcFmt, src_wstride, src_hstride);
        } else {
            rga_buf_src = wrapbuffer_virtualaddr(src, srcWidth, srcHeight, srcFmt, src_wstride, src_hstride);
        }
    }
    ...
    if (use_handle) {
        ...
        rga_buf_dst = wrapbuffer_handle(rga_handle_dst, dstWidth, dstHeight, dstFmt, dst_wstride, dst_hstride);
    } else {
        if (dst_phy != NULL) {
            rga_buf_dst = wrapbuffer_physicaladdr(dst_phy, dstWidth, dstHeight, dstFmt, dst_wstride, dst_hstride);
        } else if (dst_fd > 0) {
            rga_buf_dst = wrapbuffer_fd(dst_fd, dstWidth, dstHeight, dstFmt, dst_wstride, dst_hstride);
        } else {
            rga_buf_dst = wrapbuffer_virtualaddr(dst, dstWidth, dstHeight, dstFmt, dst_wstride, dst_hstride);
        }
    }
