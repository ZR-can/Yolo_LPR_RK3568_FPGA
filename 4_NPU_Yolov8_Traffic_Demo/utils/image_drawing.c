...
static const unsigned char* get_ascii_char_bitmap(unsigned char ch)
{
    if (ch >= ' ' && ch <= '~') {
...
static void draw_text_c3(unsigned char* pixels, int w, int h, const char* text, int x, int y, int fontpixelsize, unsigned int color) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        const unsigned char* font_bitmap = get_ascii_char_bitmap((unsigned char)text[i]);

        if (font_bitmap != NULL) {
            const int char_w = max(fontpixelsize / 2, 1);
            const int char_h = max(fontpixelsize, 1);
            const int y_offset = (fontpixelsize * 2 - char_h) / 2;
            const int advance_x = char_w + 2;
