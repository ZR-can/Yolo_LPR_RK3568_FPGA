...
右侧面板使用三个直观模式按钮切换视频识别、图片识别和行人违法检测，不再提供 FPGA BAR0
预处理参数编辑。UI 启动和退出时仍将 FPGA 复位为旁路，模型和规则始终处理完整
`1280×720` PCIe 帧。
...
方框边界与左缘黑色边角框的固定 1280×720 视频，底部是 1280×182 系统资源监控；右侧是
640×1080 模式、扩展后的结果、运行提示和控制区。视频逐帧不做 Qt 缩放，右侧和下方 UI
则直接按 1080p 原生分辨率绘制，因此视频内叠加文字和 UI 字体都避免二次插值。
...
程序启动后点击模式按钮选择识别模式，再点击“开始显示”进入 PCIe 采集和识别；采集运行时
模式按钮锁定，点击“暂停显示”后开放模式选择。暂停后不改变模式并点击“继续显示”，会复用当前工作线程、
模型和 PCIe 文件描述符；若改选其他模式，则先完整结束并释放旧后端，再等待用户点击
“开始显示”启动新后端。
FPGA 预处理参数不再从 UI 暴露；正常启动和退出会恢复旁路、参数 128、ROI 0，异常断电或
`kill -9` 无法保证恢复。部署包中的控制脚本和工具仍可用于板端诊断。
...
- 模式区域显示并互斥选择 `视频识别 / 图片识别 / 行人违法检测` 三个按钮。
- 识别运行时模式选择锁定；暂停后可以选择其他模式，切换时旧后端先完成退出，新模式需要重新
  点击“开始显示”。
- 右侧结果表置于模式按钮下方，车牌、类型与置信度的单元格字号为 30 px、表头为 24 px、
  行高为 62 px；视频预览左缘的黑色边角框仅影响显示，不改变原始帧、叠加结果或保存图片。
...
- 正常退出时，视频和图片模式都会输出成功 `ReadFrame()` 调用耗时及相邻完整帧间隔；图片模式
  还会输出静态变化检测结束到下一次读取开始的调度延迟。三项均同时给出平均值、最大值和样本数，
  用于区分驱动读取/拷贝、FPGA 完整帧交付节拍与图片模式采集线程调度开销。
- 顶部运行/PCIe 状态的标签、数值和系统资源图文字均使用更大字号。三种模式均显示
  PCIe采集、屏幕显示和画面显示耗时，并隐藏“模型推理”帧率。“画面显示耗时”表示完整帧
  读取成功后，到后端完成显示转换、叠加并准备交给 Qt 回调之前的平均耗时，不包含异步
  YOLO/PP-OCR 推理耗时。
  “屏幕显示”只在已有实际 Qt 绘制帧率时显示为原始值加 `2.5 FPS` 的校正值，终端性能统计和
  后端原始帧率不受影响。
...
- 当前部署的 `pango_pci_driver.ko` 没有对应源码；模块反汇编确认成功 `read()` 只在MMIO完成
  状态非零时对整帧执行一次 `copy_to_user`，且没有 `mmap` 回调。可在板端不修改驱动地拆分耗时：

  ```bash
  chmod +x tools/profile_pango_read.sh
  sudo tools/profile_pango_read.sh start
  # 正常运行 Qt demo 至少30秒并优雅退出
  sudo tools/profile_pango_read.sh stop /tmp/pango-read-function-graph.txt
  ```

  输出分别给出成功 `pango_cdev_read`、`copy_to_user` 和两者差值的平均/最大耗时。如果内核没有
  将 `__arch_copy_to_user` 列为可跟踪函数，脚本会保留总read统计并明确提示无法做内部拆分。
...
- `Successful ReadFrame call average/max` 统计所有返回完整帧的 `ReadFrame()` 调用墙钟耗时，
  包含暂停期间读入 drain buffer 的成功帧；`Complete frame interval average/max` 按相邻两次
  完整帧读取完成时刻计算，因此直接反映用户态实际收到完整帧的节拍。
- `Static detection-to-next-ReadFrame delay average/max` 仅在图片模式输出，从静态变化检测结束
  计到下一次驱动读取开始；它包含本帧队列提交、采集循环推进及期间发生的线程抢占，不包含下一次
  `ReadFrame()` 调用自身耗时。
