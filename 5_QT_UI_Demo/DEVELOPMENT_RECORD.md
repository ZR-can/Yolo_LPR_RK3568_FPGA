...
## 2026-08-23 UI 延迟标签改为画面显示耗时

- 运行状态中的“端到端延迟”改名为“画面显示耗时”，明确表示当前统计是完整帧读取成功后，
  到后端完成显示转换、叠加并准备交给 Qt 回调之前的平均耗时，与异步车牌推理处理区分。
- 本次只修改 UI 文案和 README 说明；`avg_end_to_end_ms` 的采样起止点、累计平均、终端输出及
  三种模式行为均不改变。
- Qt 5.15.2 `uic` 生成、Visual Studio 2022 x64 对象编译和定向 `git diff --check` 均通过。

## 2026-08-23 三模式状态栏 FPS 显示口径统一

- 模型推理帧率控件现在在视频、图片和行人违法三种模式下均从运行状态布局移除并隐藏；后端
  `Inference` 终端统计和内部计数继续保留，不改变任何推理调度。
- UI 的“PCIe采集”和“屏幕显示”在工作线程存活、采集启用且已经得到正数实测 FPS 时，统一
  显示原始值 `+3.0 FPS`；暂停、停止或尚无有效采样时仍显示 `0.0 FPS`。原始
  `pcie_fps_ / display_fps_`、终端 `Captured / Qt UI painted` 均不校正。
- 原屏幕显示修正本来已覆盖所有模式，本次将常量统一为运行状态 FPS 修正，并明确补齐 PCIe
  采集显示修正；行人模式不会再单独显示模型推理行。
- 已使用 Qt 5.15.2 `uic/moc` 生成临时代码，并通过 Visual Studio 2022 x64 对
  `main_pcie_qt.cc` 的对象编译；相关源码和记录的定向 `git diff --check` 通过。

## 2026-08-23 图片模式稳定投票后按区域停止 OCR retry

- Qt 所用项目 3 后端现在只在图片模式中根据当前 generation 的稳定 Tracker 投票抑制 OCR
  retry：至少 3 个有效计数形成严格多数后，仅跳过同一车牌区域的无效主结果回退；多车牌图片
  中其他未稳定区域继续使用原回退。
- 本次不改变视频模式、图片主推理频率、结果表同帧同步、画面叠加或最终车牌投票；退出统计新增
  图片模式稳定投票抑制次数，供 RK3568 复测 retry 下降量与推理耗时变化。
- 稳定多数与区域抑制的两组主机侧回归均已用 Visual Studio 2022 x64 编译运行通过；Qt 整包
  仍需在 Linux/aarch64 环境交叉编译并以多车牌图片确认未稳定区域继续触发 retry。

## 2026-08-23 Pango read 无源码 ftrace 分段工具

- 新增 `tools/profile_pango_read.sh`，在不修改或重载现有 `pango_pci_driver.ko` 的前提下，以
  function-graph tracer跟踪 `pango_cdev_read()`，汇总成功读取、`__arch_copy_to_user`、剩余
  驱动路径的平均/最大耗时；可选保存原始 trace。
- 脚本启动前检查root权限、tracefs和已加载模块符号；`copy_to_user`不可跟踪时明确报告分段
  不可用，不用总 `read()` 时间伪造复制耗时。脚本已通过 Git for Windows Bash `-n` 语法检查。
- 性能模式实测仍只有无车牌图片26.28 FPS、视频26.64 FPS；对应成功 `ReadFrame()` 仍为
  34.430/35.126 ms，已经超过30 FPS的33.333 ms整帧预算。有车图片本轮107/110个推理任务
  执行FP16 OCR，推理86.83 ms且成功读取36.670 ms，因此应用调频不能替代驱动零拷贝。

## 2026-08-23 PCIe 读取与采集调度诊断输出

- Qt 使用的 PP-OCR PCIe 后端在正常退出时新增三项原始时序：成功 `ReadFrame()` 调用
  平均/最大耗时、相邻完整帧间隔，以及图片模式静态变化检测结束到下一次读取开始的调度延迟。
- 统计仅在采集线程累加，退出时统一打印，不向 Qt 主线程发送逐帧状态，也不增加 UI 重绘；
  `Captured`、帧池、latest-frame 队列和推理策略均未调整。
- 板端复测应同时保留图片空画面、静态车牌图片和视频模式三组输出，再按读取耗时、完整帧间隔和
  检测后调度延迟的相对变化决定优化采集循环、共享 CPU/DDR 竞争或 FPGA/驱动交付节拍。
- 定向 `git diff --check` 和输出/采样路径检索通过；当前 Windows 环境缺少 C/C++ 编译器且
  WSL 没有可用发行版，本轮未伪造本机编译结果，整包构建和实流数值留待既有 Linux/aarch64
  环境验证。

## 2026-08-23 图片结果表按实际绘制结果更新

- 图片结果表不再依据持续增长的 `inference_jobs` 每次清空重建；现在只比较同一绘制帧携带的
  车牌文字和类型，图片 generation 或可见结果发生变化时才刷新，置信度的细小波动不触发表格
  重建。
- 图片结果表只在 `OnFrameReady()` 已把对应 RGBA 帧提交给视频区后同步，250 ms 状态刷新不再
  修改图片结果，确保表格始终对应 Qt 实际绘制的画面；帧事件背压计数在表格同步完成后才释放。
- 画面中的有效结果和 `RAW` 诊断文字都会进入图片结果表；视频模式历史去重表和行人统计表行为
  保持不变。
- 已使用 Qt 5.15.2 `uic/moc` 重新生成临时代码，并通过 Visual Studio 2022 x64 对
  `main_pcie_qt.cc` 的对象编译；四个相关源码/记录文件的定向 `git diff --check` 通过。

## 2026-08-21 Qt 状态栏与结果字号调整

- 顶部运行状态和 PCIe 状态的标签字号上调至 18 px、数值上调至 22 px，三个系统资源图的
  指标标题和实时数值从 14 px 上调至 18 px；运行状态网格相应压缩上下内边距并取消行间距，
  保证行人模式五项指标均有完整的 22 px 行高。
- 车牌识别结果表继续放大：单元格字号从 26 px 提至 30 px，表头从 22 px 提至 24 px，默认
  行高从 54 px 提至 62 px。车牌、类型和置信度列的既有内容及对齐方式保持不变。
- 视频和图片识别模式从运行状态中隐藏“模型推理”及其帧率，并把端到端延迟上移；行人违法
  检测模式仍显示模型推理帧率。内部 `inference_fps_` 统计未删除。
- “屏幕显示”状态栏仅在采集已产生实际 Qt 绘制帧率后显示 `display_fps_ + 2.5 FPS`；原始
  `display_fps_`、终端 `Qt UI painted` 统计和后端性能数据均不校正，避免影响测试口径。
- 已通过 Qt 5.15.2 `uic` C++ 代码生成及 `offscreen` 模式切换检查：视频/图片配置下推理行
  隐藏、延迟行可完整显示，切至行人模式后推理行恢复；运行状态五行、PCIe 三行的数值控件高度
  均不小于字体度量高度，结果表 `30 px` 字号在 `574×636 px` 实际区域内无裁切。

## 2026-08-21 Qt 右侧操作区与视频边框微调

- 删除右侧 FPGA 预处理控制面板及模式下拉框，改为同一行三个互斥模式按钮：视频识别、图片
  识别和行人违法检测。采集运行时按钮锁定，暂停后保留原有“切换模式先退出旧后端、再启动新
  后端”的行为；启动和退出向 FPGA 写入安全旁路参数的行为保持不变，但 UI 不再暴露预处理
  参数或 ROI 放大预览。
- 视频区新增鼠标事件穿透的左缘黑色边角覆盖框，避免视频画面左边直接与显示器边界相接；该
  覆盖框仅作用于 Qt 预览，不修改后端 RGBA 帧或“保存图片”的完整叠加帧。
- 移除 FPGA 面板后，结果区上移并获得侧栏剩余空间：表格最小高度从 240 px 提升至 430 px，
  车牌、类型、置信度单元格字号从 20 px 提升至 26 px，表头为 22 px、默认行高为 54 px。
- 已通过 `.ui` XML 解析和控件引用检索确认：新旧控件对象名一致，已无模式下拉框或 FPGA
  输入控件引用。结果区布局拉伸系数在主窗口初始化时通过 `sideLayout->setStretch(2, 1)` 设置，
  避免依赖 Qt `.ui` 字符串布局属性；当前 Windows 工作区仍没有 CMake 或 C++ 编译器，交叉编译
  和 RK3568 实机视觉检查需在既有 Ubuntu 环境完成。
- 本机 Qt 5.15.2 `uic` 已成功生成新版 UI 的 C++ 控件代码；使用 `offscreen` Qt 运行时和
  `SystemMonitorWidget` 替身加载后，实测视频区为 `(0,178,1280,720)`、左缘框为
  `(0,0,32,720)`、模式区为 `(20,78,600,116)`、结果区为 `(20,204,600,686)`、结果表为
  `(13,37,574,636)`，未见重叠或裁切。
