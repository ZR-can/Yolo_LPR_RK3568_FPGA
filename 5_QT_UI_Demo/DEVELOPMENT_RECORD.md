...
- 蓝绿高重叠、近分候选的像素颜色证据不足时，当前图片本轮不输出结果；不再让两个候选进入
  PP-OCR 后按 GA 36、OCR 分数或绿色 8 位优先规则选色。
- 颜色证据明确的蓝/绿候选流程、`conf/NMS=0.55`、IoU `0.65` 和分差 `0.10` 均保持不变。
- 完整 6 项 CTest 全部通过。

## 2026-08-19 蓝绿冲突分差门限最终调整为 0.10

- 图片模式的像素颜色裁决与 OCR 后 Tracker 兜底统一使用检测分差 `<=0.10`；IoU `0.65`
  和颜色证据 1.25 倍要求保持不变。边界回归使用 `0.09/0.11`。
- 完整 6 项 CTest 全部通过。

## 2026-08-19 蓝绿冲突分差门限调整为 0.05

- 图片模式的蓝/绿像素颜色裁决及 OCR 后 Tracker 兜底均将“分差接近”定义由 `0.01` 放宽为
  `0.05`；高度重叠 IoU `0.65` 和颜色证据 1.25 倍要求保持不变。
- `0.04` 进入裁决、`0.06` 不强制裁决的边界回归及完整 6 项 CTest 均通过。

## 2026-08-19 图片蓝绿冲突增加像素颜色证据

- 图片 OBB 的蓝/绿高重叠、近等分候选现在先统计矫正车牌的 BGR 饱和像素；颜色证据明确时
  在 OCR 前只保留对应类别，证据不足才进入既有 OCR 后 GA 36/Tracker 兜底。
- 板端可通过 `OBB color conflict resolved/unresolved` 日志读取 IoU、分差、蓝绿证据与最终
  选择。低于最终 `conf=0.55` 的 `green=0.276` 不应在新部署包中出现。
- 颜色证据及蓝绿候选顺序回归通过，完整 6 项 CTest 全部通过。

## 2026-08-19 蓝绿同框双文字按 8 位结构裁决

- 根据板端日志复现 `blue/green=0.955` 同分、框 IoU 约 `0.89` 的颜色冲突。正确结果为绿牌
  `皖AF00166`；蓝分支因 7 位截断也被旧逻辑判成合法，造成双框双文字。
- 当前跨颜色候选保留到 PP-OCR 后，图片 Tracker 在重叠去重时对检测同分且结构均合法的
  蓝/绿冲突优先绿色 8 位结果。视频模式及不同位置多车牌不进入该规则。
- 问题样例回归已通过：不论候选顺序是蓝→绿还是绿→蓝，最终均只保留
  `皖AF00166/绿`；完整 6 项 CTest 通过。

## 2026-08-19 图片 OBB 部署阈值最终调整为 0.55

- 图片模式最终使用 `conf=0.55`、同类别旋转 NMS `0.55`；静态 Tracker 去重 `0.65`
  不变。跨颜色候选随后按上一节实测改为 OCR 后结构裁决。视频模式继续使用自身原参数。

## 2026-08-19 图片 OBB 最佳阈值同步

- 根据 epoch 54 冻结测试 F1 峰值，图片模式 OBB 使用 `conf=0.70`；同类别旋转 NMS 对齐测试
  参数为 `0.70`，跨颜色近重复框使用 `0.80`。视频模式仍使用独立微调 YOLO 参数，未改动。
- 图片 Tracker 仍保留轴对齐 IoU `0.65` 的 OCR 后去重兜底，防止量化模型中未达到旋转 IoU
  `0.80` 的重复候选进入两个显示轨迹。

## 2026-08-19 图片模式固定框稳定与双结果去重

- `main_ppocr.cc` 现把 `image_mode` 显式传入共享 Tracker；图片模式不再沿用视频运动外推，
  同一 generation 的确认轨迹也不会因推理帧号/显示帧号间隔而漂移或过期。视频入口仍使用
  默认 `static_image_mode=false`，高速运动补偿参数不变。
- 图片模式在 Tracker 建轨前过滤 IoU 高于 `0.65` 的重叠结果，并优先保留 GA 36 有效、
  PP-OCR 文本置信度更高的候选；OBB 后处理同时改为跨颜色类别旋转 NMS，避免同一车牌生成
  两个近重叠框和两块文字板。
- 主机静态回归以同帧两个重叠框和连续 `-6～+6 px` 检测扰动运行，输出始终为 1 个框，确认后
  左边界仅变化 1 px；视频 Tracker 运动预测、OBB、车牌规则、PP-OCR retry 和图片 generation
  共 6 个 CTest 全部通过。项目 5 CMake 重新配置/生成通过，仍需板端用问题图片实测。

## 2026-08-19 图片模式切换 OBB 的接口方案确认

- 图片模式将新增独立 `model/yolov8_obb.rknn` 运行时资源并传给项目 3 的 OBB pipeline；视频
  模式仍固定加载 `model/yolov8.rknn`（当前微调模型），两条模式入口不再共用检测模型路径。
- Qt 帧回调、PCIe BGR565 帧结构、图片 generation、多车牌结果表和 Tracker 接口保持不变。
  OBB 检测在后端完成旋转矫正 OCR，显示结果则以四角点的轴对齐最大包围矩形进入现有叠加器。
- 现有叠加器只提供 RGA/CPU 矩形描边，文字背景板也是水平缓存 sprite；本次不扩展旋转多边形
  与倾斜文字板，以避免图片模式之外的显示接口和逐帧叠加性能回归。
- Qt worker、主窗口和启动资源检查已增加独立 `yolov8_obb_model` 参数；只有图片入口把它传给
  `RunPpocrPcieImageDemo()`，视频入口仍把 `yolov8.rknn` 传给原 `RunPpocrPcieDemo()`。
- CMake 安装包新增 `model/yolov8_obb.rknn`，`build-linux.sh` 同时用 `cmp` 校验普通微调模型
  和图片 OBB 模型，防止两个运行时文件被错误覆盖。当前进入主机编译与接口回归阶段。
- 路由断言已通过：视频入口仍传 `yolov8_model_` 并调用普通 pipeline，图片入口只传
  `yolov8_obb_model_` 并调用 OBB pipeline。Qt 主源使用重新生成的 UI/MOC 与 MSVC 19.41
  编译通过；项目 5 CMake 在 `TARGET_SOC=rk356x` 下配置/生成通过，安装脚本已确认生成
  `model/yolov8_obb.rknn` 规则。
- 项目 3 的 OBB 合成回归及车牌规则、PP-OCR 回退、静态 generation 回归均通过；现有
  `simple_tracker_motion_test` 因测试仍调用已不存在的 3 参数 Tracker API 而无法构建，该问题
  不在本次修改文件范围。仍需在 Ubuntu 完成 aarch64 全目标交叉编译，并在 RK3568 图片模式
  核对 OBB tensor 初始化日志、倾斜车牌 OCR、最大包围矩形显示和多车牌结果表。
