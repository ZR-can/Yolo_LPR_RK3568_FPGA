...
# 定义ONNX输出路径（统一文件名）
#ONNX_SAVE_PATH = MODEL_DIR + 'lprnet7repair.onnx'
ONNX_SAVE_PATH = MODEL_DIR + 'lprnet8repair.onnx'  # 若用8字符模型则启用此行
...
class maxpool_3d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.c_stride > 1:
            # 动态获取通道数
            C = x.shape[1]
            out_c = C // self.c_stride
            # 构造固定的 1x1 卷积权重，替代切片操作
            weight = torch.zeros((out_c, C, 1, 1), device=x.device, dtype=x.dtype)
            for i in range(out_c):
                weight[i, i * self.c_stride, 0, 0] = 1.0
            
            # 使用 F.conv2d 避免在 state_dict 中产生多余的权重 key
            x = F.conv2d(x, weight)
        return x
...
class LPRNet(nn.Module):
    ...
    def forward(self, x):
            ...
            for i, f in enumerate(keep_features):
                ...
                # 1. 修复 Reshape/Transpose：调用自适应池化，直接触发 NPU 的 GlobalAveragePool 硬件电路
                m = F.adaptive_avg_pool2d(f_pow, (1, 1))
                
                # 2. 跨通道求均值：使用 1x1 卷积维持 NPU 闭环
                C = m.shape[1]
                weight = torch.full((1, C, 1, 1), 1.0 / C, device=m.device, dtype=m.dtype)
                f_mean = F.conv2d(m, weight)
                
                # 3. 修复 Div 爆炸：强制进行微型标量求倒数，随后利用 NPU 执行极速张量乘法
                f_mean_inv = torch.reciprocal(f_mean)
                f = f * f_mean_inv
            ...
            # 替换为非对称平均池化：kernel_size=(高度方向求均值, 宽度方向不操作)
            # NPU 对 AvgPool2d 拥有 100% 的硬件电路支持，0 耗时，不产生任何 Transpose
            logits = F.avg_pool2d(x, kernel_size=(4, 1), stride=(1, 1))
