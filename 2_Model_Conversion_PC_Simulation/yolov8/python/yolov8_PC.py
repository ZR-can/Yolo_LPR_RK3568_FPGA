...
# add path
# 1. 获取当前脚本所在目录：yolov8/python
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# 2. 往上跳2级，获取项目根目录：2_Model_Conversion_PC_Simulation
ROOT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "..", ".."))
# 3. 将根目录加入Python搜索路径，即可找到根目录下的py_utils
sys.path.insert(0, ROOT_DIR)

from py_utils.coco_utils import COCO_test_helper
import numpy as np
#python yolov8_PC.py --model_path ../model/best.pt --img_folder ../model/test_imgs --img_save
...
def setup_model(args):
    ...
    if model_path.endswith('.pt') or model_path.endswith('.torchscript'):
        ...
        platform = 'rknn'
        model = build_rknn_for_simulator('../model/best.onnx')
...
# PC模拟器构建 RKNN 模型
def build_rknn_for_simulator(onnx_path):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_folder', type=str, default='../model/test_imgs', help='img folder path')
