...
int preprocess_image(ppocr_rec_context_t* app_ctx,
                     const image_buffer_t* src_image,
                     const image_rect_t* roi)
{
    if (src_image == NULL || src_image->virt_addr == NULL || src_image->width <= 0 ||
        src_image->height <= 0 ||
        (src_image->format != IMAGE_FORMAT_RGB888 &&
         src_image->format != IMAGE_FORMAT_BGR565)) {
        std::printf("invalid source image for PP-OCR preprocessing\n");
        return -1;
    }

    const int bytes_per_pixel = src_image->format == IMAGE_FORMAT_RGB888 ? 3 : 2;
    const int source_stride = src_image->width_stride > 0
                                  ? src_image->width_stride
                                  : src_image->width;
    const size_t minimum_size = static_cast<size_t>(source_stride) *
                                src_image->height * bytes_per_pixel;
    if (src_image->size < static_cast<int>(minimum_size)) {
        std::printf("PP-OCR source image buffer is smaller than its stride\n");
        return -1;
    }

    cv::Rect crop(0, 0, src_image->width, src_image->height);
    if (roi != NULL) {
        const int left = std::max(0, std::min(roi->left, src_image->width));
        const int top = std::max(0, std::min(roi->top, src_image->height));
        const int right = std::max(0, std::min(roi->right, src_image->width));
        const int bottom = std::max(0, std::min(roi->bottom, src_image->height));
        if (right <= left || bottom <= top) {
            std::printf("invalid source ROI for PP-OCR preprocessing\n");
            return -1;
        }
        crop = cv::Rect(left, top, right - left, bottom - top);
    }
    ...
    if (src_image->format == IMAGE_FORMAT_RGB888) {
        cv::Mat rgb(src_image->height,
                    ...
                    source_stride * 3);
        cv::cvtColor(rgb(crop), bgr, cv::COLOR_RGB2BGR);
    } else {
        cv::Mat bgr565(src_image->height,
                       src_image->width,
                       CV_8UC2,
                       const_cast<unsigned char*>(src_image->virt_addr),
                       source_stride * 2);
        cv::cvtColor(bgr565(crop), bgr, cv::COLOR_BGR5652BGR);
    }

    const double ratio = static_cast<double>(crop.width) / crop.height;
    const int resized_width = std::max(
        1,
        std::min(app_ctx->model_width,
...
{
    return inference_ppocr_rec_model_roi(app_ctx, src_image, NULL, result, perf);
}
                                  ...
                                  ppocr_rec_perf_t* perf)
{
    ...
    int ret = preprocess_image(app_ctx, src_image, roi);
