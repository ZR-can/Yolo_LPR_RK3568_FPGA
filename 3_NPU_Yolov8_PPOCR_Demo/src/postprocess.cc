...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        const float source_x1 =
            clamp(x1, 0.0f, static_cast<float>(model_in_w)) /
            letter_box->scale;
        const float source_y1 =
            clamp(y1, 0.0f, static_cast<float>(model_in_h)) /
            letter_box->scale;
        const float source_x2 =
            clamp(x2, 0.0f, static_cast<float>(model_in_w)) /
            letter_box->scale;
        const float source_y2 =
            clamp(y2, 0.0f, static_cast<float>(model_in_h)) /
            letter_box->scale;
        od_results->results[last_count].box.left =
            static_cast<int>(floorf(source_x1));
        od_results->results[last_count].box.top =
            static_cast<int>(floorf(source_y1));
        od_results->results[last_count].box.right =
            static_cast<int>(ceilf(source_x2));
        od_results->results[last_count].box.bottom =
            static_cast<int>(ceilf(source_y2));
