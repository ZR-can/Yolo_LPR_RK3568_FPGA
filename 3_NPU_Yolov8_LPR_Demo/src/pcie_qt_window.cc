...
void UpdateDriverStats(const PcieFrameSource& source, UiStatusSnapshot* status) {
...
protected:
    void run() override {
        ...
        while (!exit_requested_.load()) {
            ...
            status.end_to_end_ms += static_cast<double>(NowMilliseconds() - frame_ready_ms);
...
MainWindow::MainWindow(const QString& yolov8_model,
                       const QString& lprnet7_model,
                       const QString& lprnet8_model,
                       QWidget* parent)
    : QMainWindow(parent),
...
void MainWindow::OnFrameReady(const QImage& image, const UiStatusSnapshot& status) {
...
void MainWindow::OnWorkerFinished(const UiStatusSnapshot& status) {
    ApplyStatus(status);
...
void MainWindow::ApplyStatus(const UiStatusSnapshot& status) {
