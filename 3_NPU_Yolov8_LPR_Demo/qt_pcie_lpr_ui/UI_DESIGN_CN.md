...
右侧 `Statistics` 文本框对应原 `main_pcie.cc` 里的 `PrintPerformance()` 思路，显示：
...
Qt 版本目前是单工作线程结构，没有原 `main_pcie.cc` 的 display/inference 队列，因此 queue drops 和 pool drops 通常为 0。保留这些字段是为了界面格式和板子状态观察方式与原 printf 版本一致。
